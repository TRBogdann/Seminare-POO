...
std::ostream& operator<<(std::ostream &os,EquationHandler& handler)
{
    char* eq=handler.getEquation();
    os<<eq;
    if(eq)
    {
        delete[] eq;
...
std::istream& operator>>(std::istream &os,EquationHandler& handler)
{
    char* buffer=new char[handler.buffer_size+1];
    os.getline(buffer,handler.buffer_size);
    handler.setEquation(buffer);
    delete[] buffer;
    return os;
}
