...
double math_functions::evalSeg(char* str, int len, char flag, std::string& error)
{
    ...
    if (checking.Checker::checkIfEcuation(str) == true) {
        EquationHandler handler(len);
        double* parameters = new double[4];
        parameters[0] = handler.EquationHandler::getCoeficientOf(2);
        parameters[1] = handler.EquationHandler::getCoeficientOf(1);
        parameters[2] = handler.EquationHandler::getCoeficientOf(0);
        parameters[3] = 0.2f;
        double degree = handler.powerOfX(str, len, handler.getUnknown());
        Equation eq(degree, handler.getUnknown(), parameters);
        if (degree == 1)
            res += eq.Equation::f_degree();
        if (degree == 2) {
            solutii sol = eq.Equation::s_degree();
            std::cout << sol.x1 << " " << sol.x2 << '\n';
            res += 0;
        }
